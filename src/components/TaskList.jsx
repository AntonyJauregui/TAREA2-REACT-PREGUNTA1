...
export const TaskList = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => setTareas(TaskListMartin)}>
        Restablecer Tareas
      </button>
      {tareas.map((tarea) => (
        <div key={tarea.id}>
          <p
            style={{
              background: `${tarea.completado === true ? "green" : "red"}`,
              color: "white",
            }}
          >
            {tarea.tarea} - {tarea.completado === true ? "COMPLETADO" : "FALTA"}
            -
          </p>
          <button onClick={() => onActualizarTarea(tarea.id)}>
            Completar Tarea
          </button>
